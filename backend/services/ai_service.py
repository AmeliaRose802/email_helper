...
class AIService:
    ...
    async def _run_sync(self, func, *args):
        ...
        # Use functools.partial to properly bind arguments for run_in_executor
    ...
    def _classify_email_sync(self, email_content: str, context: str) -> Dict[str, Any]:
        ...
        # Parse email string into dict format expected by orchestrator
        subject, sender, body = self._parse_email_headers(email_content)

        # Convert to dict format that orchestrator expects
        email_dict = {
            'subject': subject,
            'sender': sender,
            'body': body,
            'date': 'Recent'  # Default date since not always available
        }

        result = self.ai_orchestrator.classify_email_with_explanation(email_dict, learning_data=[])
