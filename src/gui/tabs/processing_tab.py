#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Processing Tab - Email processing interface."""
...
class ProcessingTab:
    ...
    def create_widgets(self):
        ...
        self.start_btn = ttk.Button(control_frame, text="Start Processing", 
                                    command=self.gui.start_email_processing,
                                    style="Accent.TButton")
        self.start_btn.pack(side=tk.LEFT, padx=5)
        
        self.cancel_btn = ttk.Button(control_frame, text="Cancel", 
                                     command=self.gui.cancel_processing, state=tk.DISABLED)
        self.cancel_btn.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(control_frame, text="📊 Accuracy Dashboard", 
                  command=self.gui.open_accuracy_dashboard).pack(side=tk.LEFT, padx=5)
    ...
    def _on_custom_count_keypress(self, event):
        self.gui.root.after_idle(lambda: self.email_count_var.set("other") if self.custom_count_entry.get().strip() else None)
